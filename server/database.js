...
tasks.run('CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY, userId STRING, title TEXT NOT NULL, description TEXT);', (err) => {
    if (err) {
...
users.run('CREATE TABLE IF NOT EXISTS users (userId STRING PRIMARY KEY, email TEXT, password TEXT);', (err) => {
    if (err) {
        console.error('Ошибка создания таблицы:', err);
    } else {
        console.log('Таблица создана успешно');
